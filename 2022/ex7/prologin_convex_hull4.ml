...
module Vec2f = struct
  ...
  *)
  ...
  let lil_left df = 
    {xf = df *. Float.sin (-.lil_epsi) ; yf = -.df *. Float.cos (-.lil_epsi)}
...
let aretesMinimales (d:int) (n:int) points_de_controles =
  ...
  let (in_points, out_points) =
    ...
    |> (fun (x,y) ->  List.sort (Vec2f.angle_compare {xf = 0.; yf = 0.} begining) x,
                      List.sort (Vec2f.angle_compare {xf = 0.; yf = 0.} begining) y)
  in
  assert (List.length in_points = List.length out_points);
  (* let convex_hull_f = Array.map Vec2.to_float convex_hull in *)
  let visible_sides_from_begining =
    let new_convex_hull = convex_hull_of ({x = 0; y= -d} :: convex_hull_l) in
    (Array.length convex_hull) - (List.length new_convex_hull) + 2
  in
  let in_points_stack = 
    match in_points with 
    | [] -> []
    | hd :: tl when Vec2f.almost_eq hd begining -> tl @ [Vec2f.lil_left df]
    | hd :: tl -> in_points
  in
  let out_points_stack = 
    match List.rev out_points with 
    | [] -> []
    | hd :: tl when Vec2f.almost_eq hd begining -> (Vec2f.lil_right df) :: (List.rev tl)
    | hd :: tl -> out_points
  in
  let max_reduction =
    let rec loop reduction res_maxred in_points_stack out_points_stack : int=
      let in_pnt_routine in_point in_rest = 
      ...
      in
      let out_pnt_routine out_point out_rest = 
        (* in_point commes before outpoint anglewise ccw *)
      ...
      | in_point :: in_rest, out_point :: out_rest ->
        if Vec2f.almost_eq in_point out_point then
          ...
          in_pnt_routine in_point in_rest
        end else begin
          assert (0 < Vec2f.angle_compare {xf = 0.; yf = 0.} begining in_point out_point);
          out_pnt_routine out_point out_rest
        end
      | in_point :: in_rest, [] ->    in_pnt_routine in_point in_rest
      | [], out_point :: out_rest ->  out_pnt_routine out_point out_rest
      | [],[] -> res_maxred
    in
    loop (visible_sides_from_begining -2 ) 0 in_points_stack out_points_stack
  in
...
(* let (in_points, out_points, in_points_stack, out_points_stack) = aretesMinimales d n (points_de_controles) *)

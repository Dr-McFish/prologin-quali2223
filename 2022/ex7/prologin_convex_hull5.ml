...
module Vec2f = struct
  ...
  *)
  ...
  let lil_down df = 
    {xf = -.df *. Float.cos (-.lil_epsi) ; yf = df *. Float.sin (-.lil_epsi)}
...
let aretesMinimales (d:int) (n:int) points_de_controles =
  ...
  let visible_sides_from_begining =
    let new_convex_hull = convex_hull_of ({x = -d; y= 0} :: convex_hull_l) in
    (Array.length convex_hull) - (List.length new_convex_hull) + 2
  in
  let in_points_stack = 
    match in_points with 
    | [] -> []
    | hd :: tl when Vec2f.almost_eq hd begining -> tl @ [Vec2f.lil_up df]
    | hd :: tl -> in_points
  in
  let out_points_stack = 
    match List.rev out_points with 
    | [] -> []
    | hd :: tl when Vec2f.almost_eq hd begining -> (Vec2f.lil_down df) :: (List.rev tl)
    | hd :: tl -> out_points
  in
  let max_reduction =
